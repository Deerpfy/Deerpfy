...
## Games
 Good to know:
- **Cybershift** released on Steam. Reflex and puzzle game set in a neon cyber world.
- **LivingHell** in development. Story-driven post-apocalyptic survival built around atmosphere, exploration, and moral choices. I lead programming, design, planning, and visual direction.
Earlier work covers an MMORPG, a Mars survival game, a low-poly tank shooter, a wave-based zombie shooter, and commissioned level design. All of it is on [adamroucka.pw](https://adamroucka.pw).

## Hardware
Raspberry Pi 5, ESP32, and several Arduino boards. Firmware, the electronics around it, and the printed parts that hold it together.
- **LED and audio visualization** ESP32-C3 firmware with a Go host driving addressable strips from a live audio signal.
- **Car audio system** original head unit casing with custom electronics: Arduino, amplifier and audio modules, Bluetooth for streaming from a phone.
- **Switchable 12V supply** Raspberry Pi GPIO driving an IRLZ44N logic level MOSFET, so 3.3V logic switches 12V fans and LED loads with no level shifter.
Also a Pi-based NAS, a heavily modified Ender 3, and enclosures and knob caps designed in Fusion 360 and printed on a Bambu A1.
...
## Background
Before the products came over a hundred C++ tools for game memory analysis and modification, delivered through a single loader I wrote. That is where the reverse engineering came from, and it is the reason BetterGuard exists.
